  ...
  run: async (client, interaction, guildData, lang) => {
    ...
    switch (action) {
      case "user": {
        ...
        const reason_ =
          interaction.options.getString("reason") || "None Reason Provided.";

        const user = await client.users.fetch(id).catch((err) => {
          /* Error */
          return interaction.editReply(`${e.error} | An error occurred.`);
        });
        ...
        if (userData.blacklisted.status === true) {
          return interaction.editReply(
            `${e.error} | This user **${user.username}** is already in \`blacklist\``
          );
        }
        ...
        interaction.editReply(
          `${e.add} | Successfully added **${user.username}** in blacklist, \`Reason:\` **${reason_}**`
        );
        break;
      }
      ...
      case "server": {
        ...
        if (serverData.blacklisted.status === true) {
          return interaction.editReply(
            `${e.error} | This server **${server.name}** is already in \`blacklist\``
          );
        }
        ...
        interaction.editReply(
          `${e.add} | Successfully added **${server.name}** in blacklist, \`Reason:\` **${reason_}**`
        );
        break;
      }
      ...
      case "list": {
        ...
        await interaction.editReply({
          content: "```Click below to choose the type you want to see.```",
          components: [button],
        });
        ...
        collector.on("collect", async (i) => {
          if (i.user.id !== interaction.user.id) {
            return i.reply({
              content: `${e.error} | error you cant use this`,
          ...
          if (i.customId === "user") {
            ...
            await i.update({
              content: t.toString() || "No blacklisted users found.",
              components: [],
            });
          }

          if (i.customId === "server") {
            const servers = await client.guildsData
              .find({ "blacklisted.status": true })
              .lean();
            ...
            await i.update({
              content: t.toString() || "No blacklisted servers found.",
              components: [],
            });
          }
        });
      ...
      case "remove": {
        ...
        if (userid) {
          ...
          if (userData.blacklisted.status === false) {
            return interaction.editReply(
              `${e.error} | This user **${user.username}** isn't on the \`blacklist\``
            );
          }
          ...
          interaction.editReply(
            `<:h3:661140977968480256> | This user **${user.username}** has been removed from the \`blacklist\``
          );
        } else if (serverid) {
          ...
          if (serverData.blacklisted.status === false) {
            return interaction.editReply(
              `${e.error} | This server **${server.name}** isn't on the \`blacklist\``
            );
          }
          ...
          interaction.editReply(
            `<:h3:661140977968480256> | This server **${server.name}** has been removed from the \`blacklist\``
          );
        } else
          return interaction.editReply(`${e.error} | select what you want`);
        break;
      }
    }
  },
