...
module.exports = {
  ...
  run: async (client, interaction, guildData, lang) => {
    ...
    const mentionstatus = guildData.plugins.mention.enabled
        ? enabled
        : disabled,
      setdm = guildData.plugins.dmwinners.enabled ? enabled : disabled,
      setrolerl = guildData.plugins.role.role
        ? `<@&${guildData.plugins.role.role}>`
        : lang.config.norole,
      setrolestatus = guildData.plugins.role.enabled ? enabled : disabled;

    const configembed = new EmbedBuilder()
      .setAuthor({
        name: interaction.guild.name + " " + lang.config.configuration,
      })
      .addFields({
        name: lang.config.language,
        value: guildData.language,
        inline: true,
      })
      .addFields({
        name: lang.config.mentiont,
        value: lang.config.status + mentionstatus,
        inline: true,
      })
      .addFields({
        name: lang.config.setdmt,
        value: lang.config.status + setdm,
        inline: true,
      })
      .addFields({
        name: lang.config.setrolet,
        value:
          lang.config.setrolede +
          setrolerl +
          "\n" +
          lang.config.status +
          setrolestatus,
