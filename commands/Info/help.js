...
module.exports = {
  ...
  run: async (client, interaction, guildData, lang) => {
    ...
    if (helpcmd) {
      ...
      categorie = cmd.group.split(path.sep)[
        parseInt(cmd.group.split(path.sep).length - 1, 10)
      ];
      ...
      let embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: lang.help.sup,
            value: `[Click Here](${config.links.supportserver})`,
          }
        )
        .setTimestamp()
        .setImage("https://b.top4top.io/p_2534jc6x11.png")
      ...
      let owner = new EmbedBuilder()
        ...
        .addFields({ name: `blacklist:`, value: lang.owner.blacklistt })
        .setTimestamp()
        .setImage("https://b.top4top.io/p_2534jc6x11.png")
        .setFooter({
          text: config.embeds.footers,
      ...
      collector.on("collect", (interaction) => {
        ...
        if (interaction.values[0] === "config") {
          interaction
            .update({ embeds: [configcmd], components: components(false) })
            .catch((e) => {});
        }
        if (interaction.values[0] === "info") {
          interaction
            .update({ embeds: [info], components: components(false) })
            .catch((e) => {});
        }
        if (interaction.values[0] === "owner") {
          interaction
            .update({ embeds: [owner], components: components(false) })
            .catch((e) => {});
        }
