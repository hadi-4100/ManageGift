...
module.exports = async (client, interaction) => {
  ...
  if (interaction.isCommand()) {
    ...
    if (cd.has(interaction.user.id)) {
      return (
        interaction.reply(lang.cmd.cooldown),
    ...
    if (userData.blacklisted.status === true) {
      return interaction.reply({
        content: lang.blacklist.user(ureason),
        flags: MessageFlags.Ephemeral,
      });
    } else if (guildData.blacklisted.status === true) {
      return interaction.reply({
        content: lang.blacklist.guild(sreason),
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    client.log(
      `${interaction.user.username} id:(${interaction.user.id}) Use a command ${interaction.commandName}`,
      "log"
    );
