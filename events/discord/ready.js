...
module.exports = async (client) => {
  ...
  // start log
  client.log(
    `${client.user.tag}, ready to serve ${client.guilds.cache.reduce(
      (uss, g) => uss + g.memberCount,
      0
  ...
  setInterval(function () {
    ...
    const ac_giveaways = client.manager.giveaways.filter(
        (g) => g.ended !== true && g.pauseOptions.isPaused !== true
      ),
      ac_giveawayss = ac_giveaways.length;

    const toDisplay = status[parseInt(i, 10)].name
      .replace("{guild}", client.guilds.cache.size)
      .replace("{version}", version)
      .replace("{total_giveaways}", client.manager.giveaways.length)
      .replace("{ac_giveaways}", ac_giveawayss);
